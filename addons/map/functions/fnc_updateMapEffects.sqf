...
if (GVAR(mapShake)) then {
    ...
    if (_speed > 0.1) then {
        if (ctrlMapAnimDone _mapCtrlCtrl) then {

            _amplitude = (_speed - 0.1) / 5 * (1000 * _mapCtrlScale);
            _time = 0.1;
            ...
            _mapCtrlCtrl ctrlMapAnimAdd [_time, _mapCtrlScale, _shakePos];
            ctrlMapAnimCommit _mapCtrlCtrl;
        ...
        if (GVAR(isShaking)) then {
            ...
            ctrlMapAnimClear _mapCtrlCtrl;
            _mapCtrlCtrl ctrlMapAnimAdd [0, _mapCtrlScale, GVAR(lastStillPosition)];
            ctrlMapAnimCommit _mapCtrlCtrl;
            GVAR(isShaking) = false;
        } else {
            // The map is still, store state
            GVAR(lastStillPosition) = _mapCtrlCentre;
...
if (GVAR(mapLimitZoom)) then {
    if (GVAR(minMapSize) >= _mapCtrlScale) then {
        ctrlMapAnimClear _mapCtrlCtrl;
        _mapCtrlCtrl ctrlMapAnimAdd [0, GVAR(minMapSize) + 0.001, _mapCtrlCentre];
        ctrlMapAnimCommit _mapCtrlCtrl;
    };
};